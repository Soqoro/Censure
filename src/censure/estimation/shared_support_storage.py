"""Atomic chunk persistence for shared-support OPE experiments."""
...
def _read(path: Path) -> Any:
    ...
    if not path.is_file() or not digest_path.is_file():
        raise CorruptArtifactError(f"shared-support artifact is absent: {path}")
    raw = path.read_bytes()
    if hashlib.sha256(raw).hexdigest() != digest_path.read_text(encoding="utf-8").strip():
        raise CorruptArtifactError(f"shared-support artifact checksum mismatch: {path}")
    try:
        return json.loads(raw)
    except json.JSONDecodeError as exc:
        raise CorruptArtifactError(f"shared-support artifact is invalid JSON: {path}") from exc


class SharedSupportRunStore:
    ...
    def _cell_root(self, spec: SharedSupportCellSpec) -> Path:
    ...
    def write_catalog(self, catalog: Any) -> str:
        ...
        if path.exists():
            existing = _read(path)
            if existing != payload:
                raise FileExistsError("the existing shared-support catalog differs")
            return hashlib.sha256(path.read_bytes()).hexdigest()
        return _write(path, payload)

    def write_cell_spec(self, spec: SharedSupportCellSpec) -> str:
        path = self._cell_root(spec) / "cell_spec.json"
        if path.exists():
            existing = SharedSupportCellSpec.model_validate(_read(path))
            if existing != spec:
                raise FileExistsError("the existing shared-support cell differs")
            return hashlib.sha256(path.read_bytes()).hexdigest()
        return _write(path, spec)

    def _chunk_path(self, spec: SharedSupportCellSpec, chunk_index: int) -> Path:
    ...
    def write_chunk(
        ...
        *,
        ...
        rows: tuple[SharedSupportReplicateResult, ...],
    ) -> str:
        ...
        if len(rows) != len(expected) or {row.repetition_index for row in rows} != set(expected):
            raise ValueError("shared-support chunk has the wrong repetition set")
        if any(row.cell_id != spec.cell_id for row in rows):
            raise ValueError("shared-support chunk contains the wrong cell")
        payload = {
            "schema_version": "censure.shared-support-chunk.v1",
        ...
        if path.exists():
            existing = _read(path)
            if existing != payload:
                raise FileExistsError("the completed shared-support chunk differs")
            return hashlib.sha256(path.read_bytes()).hexdigest()
        return _write(path, payload)
    ...
    def is_chunk_complete(
        ...
        *,
        ...
        try:
            ...
            return bool(
                isinstance(raw, dict)
                and raw.get("schema_version") == "censure.shared-support-chunk.v1"
    ...
    def read_chunk(
        ...
        *,
        ...
        if not self.is_chunk_complete(
            ...
            raise CorruptArtifactError("shared-support chunk is absent, corrupt, or incomplete")
        raw = _read(self._chunk_path(spec, chunk_index))
        return tuple(SharedSupportReplicateResult.model_validate(row) for row in raw["rows"])

    def read_completed_cell(
        self, spec: SharedSupportCellSpec, *, repetitions_per_chunk: int
    ) -> tuple[SharedSupportReplicateResult, ...]:
        rows: list[SharedSupportReplicateResult] = []
        missing: list[int] = []
        ...
        if missing:
            raise FileNotFoundError(
                f"shared-support cell {spec.cell_id} is missing {len(missing)} chunks"
            )
        return tuple(rows)

    def write_summary(self, spec: SharedSupportCellSpec, summary: SharedSupportSummary) -> str:
        if summary.cell_id != spec.cell_id:
            raise ValueError("shared-support summary contains the wrong cell")
        return _write(self._cell_root(spec) / "summary.json", summary)


__all__ = ["SharedSupportRunStore"]
